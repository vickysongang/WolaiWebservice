...
func GetCourseDetailTeacherUpgrade(courseId, studentId int64) (int64, *courseDetailTeacher) {
	...
	if course.Type == models.COURSE_TYPE_DELUXE {
		status, course := GetDeluxeCourseDetailTeacher(studentId, course)
		return status, course
	} else if course.Type == models.COURSE_TYPE_AUDITION {
		status, course := GetAuditionCourseDetailTeacher(studentId, course)
		return status, course
	}
	return 0, nil
}

func GetDeluxeCourseDetailTeacher(studentId int64, course *models.Course) (int64, *courseDetailTeacher) {
	o := orm.NewOrm()
	courseId := course.Id
...
func GetAuditionCourseDetailTeacher(studentId int64, course *models.Course) (int64, *courseDetailTeacher) {
	o := orm.NewOrm()
	courseId := course.Id
	var auditionoRecord models.CourseAuditionRecord
	err := o.QueryTable("course_audition_record").Filter("user_id", studentId).Filter("course_id", courseId).
		One(&auditionoRecord)
	if err != nil {
		return 2, nil
	}
